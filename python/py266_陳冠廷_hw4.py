...
def area_to_zip(area):
	for i in zipcode:
		print('您找的郵遞區號:%s -%s' % (i, zipcode.get(i).get(area, '無資料')))
def zip_to_area(zip):
	pass
keyinQ1 = ''
keyinQ2 = ''
keyinQ1 = input('請輸入您欲搜尋郵遞區號方式 1.列出城市與區域名稱 2.用區域名稱或是郵遞區號搜尋:')
if keyinQ1 == '1':
	temp = zipcode.copy()
	templist = list(temp)
	templist.sort()
	...
	for i in range(len(templist)):
		questionstr = questionstr + str(i + 1) + '.' + templist[i] + ' '
	questionstr = questionstr + '):'
	keyinQ2 = int(input(questionstr)) - 1
	list_zip(templist[keyinQ2])
#	temp = temp.get(templist[keyinQ2]).copy()
#	templist = list(temp)
#	questionstr = '請輸入您要找的區域(A.全部 '
#	for i in range(len(temp)):
#		questionstr = questionstr + str(i + 1) + '.' + templist[i] + ' '
#	questionstr = questionstr + '):'
#	keyinQ2 = int(input(questionstr)) - 1
#	print('%s的郵遞區號是: %d' % (templist[keyinQ2], temp.get(templist[keyinQ2])))
elif keyinQ1 == '2':
	#temp = zipcode.copy()
	keyinQ2 = input('請輸入區域名稱或是郵遞區號:')
	if keyinQ2.isdigit() == True:
		zip_to_area(keyinQ2)
	else:
		area_to_zip(keyinQ2)
			#print(i)
			#print(temp.get(i).get(keyinQ2, '無資料'))
else:
	print('Error')
