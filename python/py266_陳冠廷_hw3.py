...
while FindWord != 'Q':
	if FindWord == 'Q':
		break
	FindWord = input('請輸入欲尋找的字串(或輸入Q離開):')
	n1 = 0	#text used
	n2 = 0	#newtext used
	ChapRow = 1
	ChapWord = 0
	strlenth = len(FindWord)
	while n1 >= 0:
		if text.find(FindWord, 0) == -1:
			print('查無資料')
			break
		tempn = n1
		n1 = text.find(FindWord, n1)
		n2 = newtext.find(FindWord, n2)
		ChapRow = ChapRow + text.count('\n', tempn, n1)
		tempn = 0
		for i in range(1, ChapRow):
			tempn = text.find('\n', tempn) + 1
		ChapWord = n1 - tempn + 1
		if n1 >= 0:
			print(text.find(FindWord, n1), ', ',
			      '第%d行, 第%d字' %(ChapRow, ChapWord), 
			      '...', newtext[n2-4:n2], '[', newtext[n2:n2+strlenth], ']', newtext[n2+strlenth: n2+strlenth+4], '...', sep = '')
			n1 = n1 + 1
			n2 = n2 + 1
